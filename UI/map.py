...
function initialize() {
    ...
    var cluj = new google.maps.LatLng(46.76599,23.5920281);
    var myOptions = {
        center: cluj,
        zoom: 8
    };
...
class QGoogleMap(QtWebEngineWidgets.QWebEngineView):
    ...
    def __init__(self, api_key, parent=None):
        ...
        html = HTML.replace("API_KEY", firebaseConfig.firebaseConfig.get("apiKey"))
        self.setHtml(html)
...
def setupMap(parent):
    API_KEY = firebaseConfig.firebaseConfig.get("apiKey")
    w = QGoogleMap(API_KEY, parent)
    w.resize(800, 350)
    w.show()
    w.waitUntilReady()
    w.setZoom(13)
